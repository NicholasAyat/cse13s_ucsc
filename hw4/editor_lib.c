...
ll_text *append_text(ll_text *list, char *text) {
    ll_text *new_node = calloc(1, sizeof(ll_text));
...
int ll_text_length(ll_text *list) {
    int count = 0;
    for (ll_text *current = list; current; current = current->next) {
        count++;
    }
    return count;
}
...
ll_text *duplicate_ll_text(ll_text *list) {
    ...
    ll_text *new_list = calloc(1, sizeof(ll_text));
    new_list->text = strdup(list->text);
    new_list->next = duplicate_ll_text(list->next);
    return new_list;
}
...
ll_text_stack *push_duplicate(ll_text_stack *stack) {
    ll_text_stack *new_stack = calloc(1, sizeof(ll_text_stack));
